#!/usr/bin/env bash
# perf_ab_176k.sh — 修复后 176k prefill 性能验收 (对照旧基线 sm70 394.66 / stock 278 tok/s)
set -euo pipefail
cd "$(dirname "$0")/.."
...
run_phase() { # $1 = sm70 (1|0), $2 = log
  ...
  "$BIN" -m "$MODEL" --main-gpu 0 -c 229376 -b 4096 -ub 512 \
...
run_phase 1 /tmp/perf176k_on.log
run_phase 0 /tmp/perf176k_off.log

echo "=== ON  (sm70, fixed, 176k) ==="
grep -a "prompt eval" /tmp/perf176k_on.log | tail -1
echo "=== OFF (stock, 176k) ==="
grep -a "prompt eval" /tmp/perf176k_off.log | tail -1
